class LedgersController < ApplicationController
  ...
  def create
    ...
    current_group.ledgers.create(ledger_params[:ledgers])

    respond_to do |format|
      # if @ledger.save
        format.html { redirect_to :back, notice: 'A New Transaction was successfully created.' }
  ...
  def report_kpi
   ...
   #HAshes
    ...
    @wealth_assets = @account_hash_bs.values.flatten.map(&:ammount).inject(0, &:+)/(graph_assets.inject(:+) + 0.0001) *100  #No zeros
